...
#define STAT_ASSERT(c, r, rm, w, wm)                                           \
  do {                                                                         \
...
static void test_simple(avdark_cache_t *cache, avdc_access_type_t type) {
  int i;
  int hits = 0;
  int misses = 0;
...
int main(int argc, char *argv[]) {
  ...
  assert(cache->stat_data_write == 0 && cache->stat_data_write_miss == 0 &&
         cache->stat_data_read == 0 && cache->stat_data_read_miss == 0);

  assert(cache->size == 512 && cache->block_size == 64 && cache->assoc == 1);
