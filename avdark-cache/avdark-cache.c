...
int
is_hit(avdark_cache_t* self, avdc_pa_t pa, avdc_tag_t tag, int* index)
{
	*index = index_from_pa(self, pa);
	for (int ix = 0; ix < self->assoc; ++ix)
	{
		int hit = self->lines[(*index) + ix].valid && self->lines[(*index) + ix].tag == tag;
		if (hit) {
			*index += ix;
			return hit;
		}
	...
	for (int ix = 1; ix < self->assoc; ++ix)
	{
		int ix_index = (*index) + ix;
		if (self->lines[ix_index].lru < self->lines[out_index].lru)
		{
			out_index = ix_index;
		}
	}

	self->lines[out_index].lru++;
	*index = out_index;
...
avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type)
{
	...
	int index;
	int hit = is_hit(self, pa, tag, &index);
