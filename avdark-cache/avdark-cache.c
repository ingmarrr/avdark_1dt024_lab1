 ...
 */
 ...
 static int find_lru_index(avdc_cache_line_t *set, int assoc) {
         int max_lru = -1, lru_index = 0;
         for (int i = 0; i < assoc; i++) {
             if (set->valid[i] == 0) return i;  // Use first empty slot, if not vaild
             if (set->lru[i] > max_lru) {     // Find highest index value
                 max_lru = set->lru[i];
                 lru_index = i;
             }
         }
 ...
 avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type)
 {
         ...
         if (!hit) {
                 int lru_index = find_lru_index(set, self->assoc);
                 set->tags[lru_index] = tag;
                 set->valid[lru_index] = 1;
                 hit_index = lru_index;
             }
         
             // LRU update: the accessed way gets LRU value 0, others increment
         for (int i = 0; i < self->assoc; i++) {
                 if (i == hit_index) set->lru[i] = 0;
                 else set->lru[i]++;
             }
 ...
 avdc_flush_cache(avdark_cache_t *self)
 {
         ...
         for (int i = 0; i < self->number_of_sets; i++) {
                 for (int j= 0; j < self->assoc; j++) {
                         self->lines[i].valid[j] = 0;
                         self->lines[i].tags[j] = 0;
                         self->lines[i].lru[j]=self->assoc-1;
                 }
             ...
             avdc_size_t size, avdc_block_size_t block_size, avdc_assoc_t assoc)
 {
         ...
         for (int i = 0; i < self->number_of_sets; i++) {
                 self->lines[i].tags = AVDC_MALLOC(assoc, avdc_tag_t);
                 self->lines[i].valid = AVDC_MALLOC(assoc, int);
                 self->lines[i].lru = AVDC_MALLOC(assoc, int);
             }
 ...
 avdc_print_internals(avdark_cache_t *self)
 {
         ...
         for (i = 0; i < self->number_of_sets; i++)
                 fprintf(stderr, "tag: <0x%.16lx> valid: %ls\n",
                         (long unsigned int)self->lines[i].tags,
                         self->lines[i].valid);
 }
 ...
 avdc_delete(avdark_cache_t *self)
 {
         if (self->lines) {
                 for (int i = 0; i < self->number_of_sets; i++) {
                     AVDC_FREE(self->lines[i].tags);
                     AVDC_FREE(self->lines[i].valid);
                     AVDC_FREE(self->lines[i].lru);
                 }
                 AVDC_FREE(self->lines);
             }
