...
void avdc_access(avdark_cache_t *self, avdc_pa_t pa, avdc_access_type_t type) {
  ...
  if (!hit) {
    for (int i = 0; i < self->assoc; i++) {
      if (self->lines[index + i].lru < self->lines[index + toReplace].lru) {
        toReplace = i;
      }
    }
    self->lines[index + toReplace].valid = 1;
    self->lines[index + toReplace].tag = tag;
    self->lines[index + toReplace].lru = clock;
  }
  // avdc_dbg_log(self, "cache update: pa: 0x%.16lx, tag: 0x%.16lx, index: %d,
  // hit: %d, lru: %d, toReplace %d\n",
  //         (unsigned long)pa, (unsigned long)tag, index, hit, lru, toReplace);
  // avdc_dbg_log(self, "cache set: valid: %d, tag: 0x%.16lx, lru: %d\n",
  //         self->lines[index+0].valid, (unsigned
  //         long)self->lines[index+0].tag, self->lines[index+0].lru);
  // avdc_dbg_log(self, "cache set: valid: %d, tag: 0x%.16lx, lru: %d\n",
  //         self->lines[index+1].valid, (unsigned
  //         long)self->lines[index+1].tag, self->lines[index+1].lru);
...
int avdc_resize(avdark_cache_t *self, avdc_size_t size,
   ...
   * array is allocated. */
  self->lines =
      AVDC_MALLOC(self->number_of_sets * self->assoc, avdc_cache_line_t);
...
void avdc_print_internals(avdark_cache_t *self) {
  ...
  for (i = 0; i < self->number_of_sets; i++)
    fprintf(stderr, "tag: <0x%.16lx> valid: %d\n",
            (long unsigned int)self->lines[i].tag, self->lines[i].valid);
}
